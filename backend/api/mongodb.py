...
# ---------------------------------------------------------------------------
# 1. DNS patch -- must run BEFORE MongoClient is created
# ---------------------------------------------------------------------------

def _patch_dns():
    """
    Override dnspython's resolver to use public DNS (Google + Cloudflare)
    instead of the local resolver, which often times out on restricted networks.
    """
    try:
        import dns.resolver
        ...
        print("[DNS] Patched -> using Google/Cloudflare DNS for MongoDB Atlas SRV lookup")
        return True
    except ImportError:
        print("[WARN] dnspython not installed -- run: pip install dnspython")
        return False
    except Exception as exc:
        print(f"[WARN] DNS patch failed ({exc}) -- continuing with system DNS")
        return False
